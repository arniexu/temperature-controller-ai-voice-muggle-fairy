...
static void xpt2046_bb_delay(void)
{
...
static uint8_t xpt2046_bb_xfer8(uint8_t tx)
{
	uint8_t rx = 0;
	for(uint8_t i = 0; i < 8U; i++) {
		HAL_GPIO_WritePin(XPT2046_MOSI_Port, XPT2046_MOSI_Pin, (tx & 0x80U) ? GPIO_PIN_SET : GPIO_PIN_RESET);
		HAL_GPIO_WritePin(XPT2046_SCK_Port, XPT2046_SCK_Pin, GPIO_PIN_SET);
		xpt2046_bb_delay();
		rx <<= 1;
		if(HAL_GPIO_ReadPin(XPT2046_MISO_Port, XPT2046_MISO_Pin) == GPIO_PIN_SET) {
			rx |= 1U;
		}
		HAL_GPIO_WritePin(XPT2046_SCK_Port, XPT2046_SCK_Pin, GPIO_PIN_RESET);
		xpt2046_bb_delay();
		tx <<= 1;
	}
	return rx;
...
#else
		(void)xpt2046_bb_xfer8(command);
		receiveByteX[0] = xpt2046_bb_xfer8(0x00);
		receiveByteX[1] = xpt2046_bb_xfer8(0x00);
#endif
...
#if XPT2046_USE_HAL_SPI
		uint8_t zerosY[2] = { 0x00,0x00};
		HAL_SPI_Transmit(spiPort,&command,sizeof(command),HAL_MAX_DELAY);
		HAL_SPI_TransmitReceive(spiPort,(uint8_t*)zerosY,receiveByteY,sizeof(receiveByteY),HAL_MAX_DELAY);
#else
		(void)xpt2046_bb_xfer8(command);
		receiveByteY[0] = xpt2046_bb_xfer8(0x00);
		receiveByteY[1] = xpt2046_bb_xfer8(0x00);
#endif
...
#if XPT2046_USE_HAL_SPI
		uint8_t zerosZ1[2] = { 0x00,0x00};
		HAL_SPI_Transmit(spiPort,&command,sizeof(command),HAL_MAX_DELAY);
		HAL_SPI_TransmitReceive(spiPort,(uint8_t*)zerosZ1,receiveByteZ1,sizeof(receiveByteZ1),HAL_MAX_DELAY);
#else
		(void)xpt2046_bb_xfer8(command);
		receiveByteZ1[0] = xpt2046_bb_xfer8(0x00);
		receiveByteZ1[1] = xpt2046_bb_xfer8(0x00);
#endif
...
#if XPT2046_USE_HAL_SPI
		uint8_t zerosZ2[2] = { 0x00,0x00};
		HAL_SPI_Transmit(spiPort,&command,sizeof(command),HAL_MAX_DELAY);
		HAL_SPI_TransmitReceive(spiPort,(uint8_t*)zerosZ2,receiveByteZ2,sizeof(receiveByteZ2),HAL_MAX_DELAY);
#else
		(void)xpt2046_bb_xfer8(command);
		receiveByteZ2[0] = xpt2046_bb_xfer8(0x00);
		receiveByteZ2[1] = xpt2046_bb_xfer8(0x00);
#endif
