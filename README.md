...
| 受众 | 你能从中学到什么 |
|------|-----------------|
| **入门 1~2 年的 STM32 开发者** | 真正能落地的工程结构（不是教程里的玩具 main.c）、五层分层是怎么拆的、LVGL 怎么做动画和特效、FSMC 驱屏实战、FreeRTOS 任务设计 |
| **有经验的嵌入式开发者（2 年+）** | BSP 抽象与硬件解耦思路、LVGL 自定义 Widget 开发、粒子特效优化技巧、如何把你的硬件接入这个框架 |
...
如果你只有 1~2 年经验，下面几个技术你可能还没碰过。**这不是你的问题——大部分 STM32 教程不会教你这些。**

| 可能会卡住的地方 | 难度 | 为什么难 | 怎么办 |
|:--|:--:|:--|:--|
| **FSMC 并口驱动 ILI9341** | 🔴 | 不像 SPI 那样"发字节就亮"——FSMC 是总线控制器，你得理解时序配置、地址映射、NE 片选 | 先把 CubeMX FSMC 配置截图和自己开发板的 FSMC 引脚对比一下；项目中 `fsmc.c` 是 CubeMX 生成的，**你不用改它** |
| **LVGL 框架** | 🔴 | LVGL 有自己的渲染模型（display driver → flush callback → timer handler），第一次看会懵 | 看 2 个文件就够了：`display_drv.c`（怎么把像素推到 LCD）、`bsp_stubs.c`（LCD/触摸的底层操作）。剩下的 LVGL API 边用边查 |
| **git submodule 拉取 LVGL** | 🟡 | `git clone` 完后 LVGL 目录是空的，不报错但是编译会缺文件 | 执行 `git submodule update --init` 就行，README 里写了但很多人会漏掉 |
| **FreeRTOS 任务设计** | 🟡 | 本项目只用了一个任务（UI 任务），但理解 `osKernelStart()` 之后代码流到哪去了需要一点时间 | 打开 `freertos.c`，找到那个 UI 任务函数，看它的 `for(;;)` 循环——**这就是"程序的主循环"** |
| **ARM Compiler 6 (AC6)** | 🟡 | 如果你之前用 AC5，AC6 的编译错误信息不太一样，报错更多 | 项目已经配置好 AC6，直接编译就行。如果报错先看是不是 Pack 没装 |
...
我是一名有 10 年经验的 **BMC 固件工程师**——所以你会看到代码里有严格的硬件抽象分层、启动诊断钩子、`bsp_stubs.c` 一文件替换适配、以及注释里大量"为什么这么设计"的思考。但你不用被 10 年吓到——因为 F407、FreeRTOS、LVGL、FSMC 对我来说全都是**第一次**。我就是在用"10 年的软件习惯"去学一套全新的硬件技术栈，踩过的坑都写下来了。
...
> 🔨 **以下内容尚未完成，仅作为路线图预告。标"筹备中"意味着目前还不存在——请不要现在发邮件或开 Issue 询问。**

### 成品板

计划推出**预烧录固件的成品开发板**（启明欣欣 STM32F407 高配版 V6.1），到手即用，拆包上电就能看到粒子动画。如果你不想自己配置 Keil、编译、烧录，这是最快体验本项目的方式。
...
| 平台 | 链接 | 内容 |
|------|------|------|
| **知乎** | [zhihu.com/people/modugoudaner](https://www.zhihu.com/people/modugoudaner) | 嵌入式技术分享 |
| **抖音** | 抖音号 `64419630668` | 本项目演示视频 |
| **B 站** | 筹备中 | 配套视频课程（预计 2026 Q3-Q4） |
